...
function SuccessInner() {
  ...
  async function mintUnlock() {
    ...
    localStorage.setItem("listcheck_unlock", data.token);
    localStorage.setItem("listcheck_plan", info.plan);
    setDone(true);
    setStatus(
      info.plan === "pro"
        ? "Pro active for 30 days on this browser. Go home and check again — no watermark."
        : "Credits pack active: 100 clean reports on this browser. Go home and check again."
    );
  }
    ...
    <main className="min-h-screen bg-zinc-50 px-6 py-16 text-zinc-900">
      <div className="mx-auto max-w-lg rounded-2xl border border-zinc-200 bg-white p-8 shadow-sm">
        ...
        <p className="mt-2 text-zinc-600">
          {info.plan === "pro"
            ? "ListCheck Pro — clean reports for 30 days on this browser."
            : "ListCheck Credits — 100 clean (no-watermark) reports on this browser."}
        </p>
        {info.orderId && (
