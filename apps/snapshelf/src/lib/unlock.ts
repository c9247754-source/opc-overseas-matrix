...
export type UnlockPayload = {
  ...
  plan: UnlockPlan;
  exp: number;
  /** Remaining clean exports for credits plan; unused for pro. */
  creditsLeft?: number;
};
...
/** Pro: 30 days unlimited. Credits: 100 clean exports (or until exp, 1 year). */
...
export function verifyUnlockToken(
  ...
  try {
    ...
    // New format: email|product|plan|exp|creditsLeft|sig
    if (parts.length === 6) {
    ...
    // Legacy format: email|product|exp|sig → treat as 30-day pro-like unlock
    if (parts.length === 4) {
...
/** After a successful clean export on credits plan, mint a token with credits-1. */
